...
const Home = () => {
  ...
  const imageListReducerState = useSelector(state => state.imageListReducerState);
  const dispatch = useDispatch();

  useEffect(() => {
    // dispatch(getImageListFromAPI());
    console.log(`imageListReducerState: ${JSON.stringify(imageListReducerState)}`);
  }, []);
...
export default connect(mapStateToProps, { getImageListFromAPI })(Home);